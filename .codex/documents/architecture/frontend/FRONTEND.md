...
- The frontend uses **React** (via ReactRoblox) for UI rendering, **Charm** atoms for state management, and **Charm-sync** to receive replicated state from the server.
- It follows a **Feature-Sliced Design** where each feature is a self-contained slice with three internal layers.
...
- [DDD.md](DDD.md) - DDD-style controller, application, and infrastructure context structure
...
- The Counter feature at `StarterPlayerScripts/Contexts/Counter/` is the canonical working example of all frontend patterns.
- Copy its structure when creating new features.
