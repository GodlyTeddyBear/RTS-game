...
-- Required forge building must exist on player's lot
local RequiredForgeBuilding = Spec.new("RequiredBuildingMissing", Errors.REQUIRED_BUILDING_MISSING,
	function(c: TCraftItemCandidate) return c.HasRequiredForgeBuilding end
)
...
return table.freeze({
	CanCraftItem = RecipeExists:And(RecipeUnlocked):And(RequiredForgeBuilding):And(SufficientMaterials):And(InstantCraftRecipe),
})
