...
function WaveEntityFactory:Init(registry: any, _name: string)
	self._componentRegistry = registry:Get("WaveComponentRegistry")
	BaseECSEntityFactory.InitBase(self, registry, "WaveComponentRegistry")
...
function WaveEntityFactory:_EnsureSessionEntity(): number
	...
	if sessionEntity ~= nil and self._world:exists(sessionEntity) then
		return sessionEntity
	end

	local entity = self._world:entity()
	self._world:set(entity, components.RuntimeStateComponent, _defaultState())
	self._world:add(entity, components.SessionTag)
...
function WaveEntityFactory:GetStateReadOnly(): WaveRuntimeState
	...
	local currentState = self._world:get(entity, components.RuntimeStateComponent)
	if currentState == nil then
		currentState = _defaultState()
		self._world:set(entity, components.RuntimeStateComponent, currentState)
	end
...
function WaveEntityFactory:SetState(nextState: WaveRuntimeState)
	...
	local entity = self:_EnsureSessionEntity()
	self._world:set(entity, components.RuntimeStateComponent, _cloneState(nextState))
end
