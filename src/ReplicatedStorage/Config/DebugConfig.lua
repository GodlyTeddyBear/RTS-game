...
return table.freeze({
	...
	SCHEDULER_INTERVAL = 5 / 60, -- Seconds between server scheduler ticks (testing only)
	...
	COMBAT_MOVEMENT_PIPELINE_STAGE_RESERVE_SECONDS = 1 * MILLISECOND, -- Reserve budget that makes the movement pipeline defer later stages to the next tick (testing only)
