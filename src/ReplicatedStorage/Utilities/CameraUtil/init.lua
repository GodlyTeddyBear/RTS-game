...
function CameraUtil:MoveTo(target: BasePartOrCFrame, duration: Number, style: EasingStyle, direction: EasingDirection)
	...
	if duration == 0 then
		...
		self.CurrentTween =
			TweenService:Create(self.Camera, TweenInfo.new(duration, style, direction), { CFrame = target })
...
function CameraUtil:SetFOV(fov: Number, duration: Number, style: EasingStyle, direction: EasingDirection)
	...
	if duration == 0 then
		...
		self.FOVTween =
			TweenService:Create(self.Camera, TweenInfo.new(duration, style, direction), { FieldOfView = fov })
		self.FOVTween:Play()
	end
end
...
function CameraUtil:Orbit(
	origin: BasePartOrCFrame,
	speed: Number,
