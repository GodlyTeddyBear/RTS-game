...
function MathFunctions.flip(x)
	...
	if typeof(x) ~= "number" then
		return warn("Make sure parameter 1 is a number from 0 to 1")
	end
...
function MathFunctions.sd(x, PopulationToggle)
	if typeof(x) ~= "table" then
		return warn("Make sure parameter 1 is a table")
	end
	if typeof(PopulationToggle) ~= "boolean" and PopulationToggle ~= nil then
		return warn("Make sure parameter 2 is set to nil or a boolean")
	end
	if PopulationToggle == nil then
	...
	if not s then
		return warn("Make sure all values in the table are numbers")
	end
	local avg = s / #x
	local t = 0
	for i, v in ipairs(x) do
...
function MathFunctions.median(x)
	...
	local median
	if index % 1 ~= 0 then
		median = (x[index - 0.5] + x[index + 0.5]) / 2
	else
		median = x[index]
	end
	return median
...
function MathFunctions.gcd(a, b)
	if typeof(a) ~= "number" and typeof(b) ~= "number" then
		return warn("Make sure parameter 1 and 2 are both numbers")
	end
	if typeof(a) ~= "number" then
		return warn("Make sure parameter 1 is a number")
	end
	if typeof(b) ~= "number" then
		return warn("Make sure parameter 2 is a number")
	end
	if b ~= 0 then
...
function MathFunctions.floor(x, NearestDecimal)
	if NearestDecimal == nil then
		NearestDecimal = 0
	end
	if typeof(x) ~= "number" and typeof(NearestDecimal) ~= "number" then
		return warn("Make sure parameter 1 and 2 are both numbers")
	end
	if typeof(x) ~= "number" then
		return warn("Make sure parameter 1 is a number")
	end
	if typeof(NearestDecimal) ~= "number" then
		return warn("Make sure parameter 2 is a number")
	end
...
function MathFunctions.iteration(Input, Iterations, Func)
	...
	if typeof(Iterations) ~= "number" then
		return warn("Make sure parameter 2 is a number")
	end
	if Iterations % 1 ~= 0 then
		return warn("Make sure parameter 2 is an integer")
	end
	if typeof(Func) ~= "function" then
		return warn("Make sure parameter 3 is a function")
	end
...
function MathFunctions.lucas(x)
	return (((1 + math.sqrt(5)) / 2) ^ x - ((1 - math.sqrt(5)) / 2) ^ x) / math.sqrt(5)
		+ (((1 + math.sqrt(5)) / 2) ^ (x - 2) - ((1 - math.sqrt(5)) / 2) ^ (x - 2)) / math.sqrt(5)
end
...
function MathFunctions.digitadd(x)
	...
	if x % 1 ~= 0 then
		return warn("Make sure parameter 1 is an integer")
	end
...
function MathFunctions.primeFactor(x, t)
	...
	local newNum = x
	while true do
		if #MathFunctions.factors(newNum) <= 2 then
			break
		end
		for i = 2, newNum ^ 0.5 do
			if newNum % i == 0 then
...
function MathFunctions.fromComma(x)
	if typeof(x) ~= "string" then
		return warn("Make sure parameter 1 is a string")
	end
...
function MathFunctions.toKMBT(x, NearestDecimal)
	...
	if neg then
		return "-"
			.. math.floor((x / (10 ^ (3 * math.floor((digits - 1) / 3)))) * 10 ^ NearestDecimal) / 10 ^ NearestDecimal
			.. suffix
	end
	return math.floor((x / (10 ^ (3 * math.floor((digits - 1) / 3)))) * 10 ^ NearestDecimal) / 10 ^ NearestDecimal
		.. suffix
end
...
function MathFunctions.fromKMBT(x)
	...
	if neg then
		return tonumber("-" .. string.split(x, letter)[1] * factor)
	end
...
function MathFunctions.toScientific(x, Base)
	...
	if typeof(Base) ~= "number" and Base ~= nil then
		return warn("Make sure parameter 2 is a number or nil")
	end
	local neg = false
	if x < 0 then
		neg = true
...
function MathFunctions.fromNumeral(x)
	...
	for char in string.gmatch(tostring(x), ".") do
		...
		if ifString == false then
			return warn("Check if you're only using characters (M,D,C,L,X,V,I)")
		end
	end
	...
	if num <= numeralLength then
		decimal += numberMap[string.sub(x, num, num)]
	end
	return decimal
end
...
function MathFunctions.fromPercent(x)
	...
	if not s then
		return warn('Make sure parameter 1 is in the form "N%"')
	end
	return n / 100
end

function MathFunctions.toFraction(x, MixedToggle)
	if MixedToggle == nil then
		MixedToggle = false
	end
	if typeof(x) ~= "number" and typeof(MixedToggle) ~= "boolean" then
		return warn("Make sure parameter 1 is a number and parameter 2 is a boolean")
	end
	if typeof(x) ~= "number" then
		return warn("Make sure parameter 1 is a number")
	end
	if typeof(MixedToggle) ~= "boolean" then
		return warn("Make sure parameter 2 is a boolean")
	end
	local neg = 1
	if x < 0 then
		x = math.abs(x)
	...
	if number < 1 / 20001 and number ~= 0 then
		if string.find(tostring(number), "e") then
			number, factor =
				tonumber(string.split(tostring(number), "e")[1]) / 10,
				10 ^ (-tonumber(string.split(tostring(number), "e")[2]) - 1)
		else
...
function MathFunctions.fromFraction(x)
	...
	local s = pcall(function()
		...
		if mixed then
			num = string.split(string.split(x, "/")[1], " ")[2]
		end
	end)
	if not s then
		return warn('Make sure parameter 1 is a string in the form of "A B/C" or A/B')
	end
	return whole + num / denom
end

function MathFunctions.toTime(x, AMPMToggle)
	if AMPMToggle == nil then
		AMPMToggle = false
	end
	if typeof(x) ~= "number" and typeof(AMPMToggle) ~= "boolean" then
		return warn("Make sure parameter 1 is a number from 0-24 and parameter 2 is a boolean")
	end
	if typeof(x) ~= "number" then
		return warn("Make sure parameter 1 is a number from 0-24")
	end
	if typeof(AMPMToggle) ~= "boolean" then
		return warn("Make sure parameter 2 is a boolean")
	end
...
function MathFunctions.fromTime(x)
	...
	local hours, minutes, seconds =
		string.split(x, ":")[1], string.split(x, ":")[2], string.split(string.split(x, ":")[3], " ")[1]
...
function MathFunctions.toBase(x, Base, CurrentBase)
	...
	if typeof(CurrentBase) ~= "number" then
		return warn("Make sure parameter 1 is a number")
	end
...
function MathFunctions.solver(f1, f2)
	...
	if typeof(f2) ~= "function" then
		local a = f2
		f2 = function(x)
			return a
		end
	end
	if typeof(f1) ~= "function" then
		local b = f1
		f1 = function(x)
			return b
		end
...
function MathFunctions.derivative(x, Function)
	if typeof(Function) ~= "function" then
		return warn("Make sure parameter 2 is a function")
	end
...
function private.div(c1, c2)
	if typeof(c1) == "number" then
		c1 = { c1, 0 }
	end
	if typeof(c2) == "number" then
		c2 = { c2, 0 }
	end
...
function Complex.sinh(x)
	return setmetatable(
		private.div(
			private.add(Complex.pow(math.exp(1), x), private.mul(-1, Complex.pow(math.exp(1), { -x[1], -x[2] }))),
			2
		),
		mt
	)
...
function Complex.toPolar(c1, StringFormat)
	...
	if typeof(StringFormat) ~= "boolean" then
		return warn("Make sure parameter 2 is a boolean or nil")
	end
...
function Complex.sin(x)
	if typeof(x) == "number" then
		return setmetatable({ math.sin(x), 0 }, mt)
	end
	local a, b = x[1], x[2]
...
function Complex.sqrt(x)
	...
	return setmetatable(Complex.pow(x, 0.5), mt)
end
...
function Complex.asin(x)
	...
	if x[1] > -1 and x[1] < 1 and x[2] == 0 then
		return { math.asin(x[1]), 0 }
	end
	local a, b = x[1], x[2]
	local z2 = private.mul(-1, Complex.pow(x, 2))
	local sqrt = Complex.pow(private.add(1, z2), 0.5)
...
function Complex.cos(x)
	if typeof(x) == "number" then
		return setmetatable({ math.cos(x), 0 }, mt)
	end
...
function Complex.atan(x)
	...
	local ans = private.mul({ 0, 0.5 }, sum)
	ans[1] += math.pi / 2
	return setmetatable(ans, mt)
end
...
function MathFunctions.fibonacci(x)
	local term1 =
		private.add(Complex.pow((1 + math.sqrt(5)) / 2, x), private.mul(-1, Complex.pow((1 - math.sqrt(5)) / 2, x)))
	if x % 1 == 0 then
		return setmetatable({ math.floor(private.div(term1, math.sqrt(5))[1]), 0 }, mt)
	else
		return setmetatable(
			{
				math.round(private.div(term1, math.sqrt(5))[1] * 1e10) / 1e10,
				(private.div(term1, math.sqrt(5))[2] * 1e10) / 1e10,
			},
			mt
		)
	end
...
function MathFunctions.tetration(x, Iteration)
	if Iteration == math.huge then
		local a = MathFunctions.solver(function(xx)
			return xx ^ (1 / xx)
		end, x)
		local newt = {}
			...
			if x[2] == 0 then
				if x[1] > 142 then
					...
					t1 = 2.5066282746310002
						* Complex.exp((x[1] - 0.5) * Complex.log(x[1] + 6.5))
						* Complex.exp(-x[1] - 6.5)
						* (
							0.99999999999980993
							+ 676.5203681218851 / x[1]
							+ -1259.1392167224028 / (x[1] + 1)
							+ 771.32342877765313 / (x[1] + 2)
							+ -176.61502916214059 / (x[1] + 3)
							+ 12.507343278686905 / (x[1] + 4)
							+ -0.13857109526572012 / (x[1] + 5)
							+ 9.9843695780195716e-6 / (x[1] + 6)
							+ 1.5056327351493116e-7 / (x[1] + 7)
						)
					if x[1] % 1 == 0 then
						t1 = math.round(t1)
					end
				...
				t1, t2 =
					t1 + 676.5203681218851 * x[1] / (x[1] ^ 2 + x[2] ^ 2),
					t2 + -676.5203681218851 * x[2] / (x[1] ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + -1259.1392167224028 * (x[1] + 1) / ((x[1] + 1) ^ 2 + x[2] ^ 2),
					t2 + 1259.1392167224028 * x[2] / ((x[1] + 1) ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + 771.32342877765313 * (x[1] + 2) / ((x[1] + 2) ^ 2 + x[2] ^ 2),
					t2 + -771.32342877765313 * x[2] / ((x[1] + 2) ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + -176.61502916214059 * (x[1] + 3) / ((x[1] + 3) ^ 2 + x[2] ^ 2),
					t2 + 176.61502916214059 * x[2] / ((x[1] + 3) ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + 12.507343278686905 * (x[1] + 4) / ((x[1] + 4) ^ 2 + x[2] ^ 2),
					t2 + -12.507343278686905 * x[2] / ((x[1] + 4) ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + -0.13857109526572012 * (x[1] + 5) / ((x[1] + 5) ^ 2 + x[2] ^ 2),
					t2 + 0.13857109526572012 * x[2] / ((x[1] + 5) ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + 9.9843695780195716e-6 * (x[1] + 6) / ((x[1] + 6) ^ 2 + x[2] ^ 2),
					t2 + -9.9843695780195716e-6 * x[2] / ((x[1] + 6) ^ 2 + x[2] ^ 2)
				t1, t2 =
					t1 + 1.5056327351493116e-7 * (x[1] + 7) / ((x[1] + 7) ^ 2 + x[2] ^ 2),
					t2 + -1.5056327351493116e-7 * x[2] / ((x[1] + 7) ^ 2 + x[2] ^ 2)
