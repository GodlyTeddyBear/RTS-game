...
function ResultApplication.ResolveIndexedPair(
	...
	local secondValue, secondIndex =
		ResultApplication.ResolveIndexedValue(row, secondIndexFieldName, valuesByIndex)
...
function ResultApplication.ApplyRows(config: TApplyRowsConfig): TRowApplicationResult
	assert(type(config) == "table", "ParallelRunner.ResultApplication.ApplyRows requires a config table")
	assert(type(config.Rows) == "table", "ParallelRunner.ResultApplication.ApplyRows requires Rows")
	assert(type(config.ApplyRow) == "function", "ParallelRunner.ResultApplication.ApplyRows requires ApplyRow")
