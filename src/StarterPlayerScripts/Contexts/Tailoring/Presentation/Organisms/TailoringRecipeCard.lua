...
export type TTailoringRecipeCardProps = {
	Recipe: TailoringRecipeViewModel.TTailoringRecipeViewModel,
	OnTailor: (recipeId: string) -> (),
	LayoutOrder: number?,
}
...
local function TailoringRecipeCard(props: TTailoringRecipeCardProps)
	...
	local btnText = if recipe.CanAfford then "Tailor" else "Need Materials"
	...
	return e("Frame", {
		...
		Inner = e(Frame, {
			...
			children = {
				...
				-- Right: output quantity + tailor button
				TailArea = e("Frame", {
					...
					TailBtn = e("TextButton", {
						...
						Text = "",
						TextSize = 1,
						[React.Event.MouseEnter] = tailHover.onMouseEnter,
						[React.Event.MouseLeave] = tailHover.onMouseLeave,
						[React.Event.Activated] = if recipe.CanAfford
							then tailHover.onActivated(function()
								props.OnTailor(recipe.Id)
							end)
							else function()
